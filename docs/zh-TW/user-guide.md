...
- **檢視** — 各圖表顯示隱藏、圖表面板、深色模式、視窗置頂、疊加視窗顯示隱藏、點選穿透
...
| 點選穿透 | 檢視 → 點選穿透。滑鼠落到遊戲上。此後疊加視窗右鍵功能表不可用，請改用檢視功能表 |
...
- **Ctrl+D** 或 檢視 → **深色模式** 切換深淺色。Windows 原生標題列會一起變。
- **Ctrl+T** 或 檢視 → **視窗置頂** 釘住主視窗。
...
2. 否則，Windows 地區若以 `zh` 開頭則使用簡體中文。
3. 其餘情況為英語。

功能表裡沒有語言開關。可在捷徑或命令列設定：
...
`config.json` 儲存主題、圖表顯示隱藏、視窗與分割列、疊加視窗穿透、監視清單與自動停止秒數。UTF-8 無 BOM。刪除即可重設（首次執行可能從舊登錄機碼還原一部分值）。
