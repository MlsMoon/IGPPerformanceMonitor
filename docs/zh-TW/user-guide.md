...
2. 左側 **處理程序選取** 面板可依名稱搜尋。
3. 選取一個或多個處理程序（例如 `game.exe`），按 **▶**，或按兩下名稱。
4. 選擇性：**自動停止（秒）**。`0` 表示一直擷取到你手動停止。
5. 按 **開始擷取** 或按 **F5**。
6. 正常遊玩。觀看統計表與圖表。每個被監視的視窗上會出現疊加視窗。
7. 按 **停止** 或再按一次 **F5**。

監視清單會儲存，下次啟動自動還原。處理程序尚未啟動也沒關係，PresentMon 會等它開始呈現幀。

圖形介面必須至少加入一個處理程序。監視整台電腦上所有處理程序只在[無介面模式](#14-無介面--命令列)可用（`--all-processes`），不適合即時介面。

若這次一幀都沒有，程式會提示：處理程序名稱可能不對，或目標並沒有在繪製。
...
用來回顧上次工作階段或無介面擷取，不必再跑 PresentMon。
