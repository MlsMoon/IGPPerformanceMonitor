...
**设置 → 点击穿透** 会把鼠标交给游戏，此时叠加窗上右键无效。到设置菜单关掉穿透，或用 **F9** 隐藏叠加窗。
...
**设置 → 语言** 可在英语和简体中文之间切换，不必重启进程。下次启动时 `IGP_LANG` 仍优先。
...
尚未保存语言时，Windows 区域以 `zh` 开头则用简体中文。繁体中文系统在界面里同样映射到 `zh_CN`。`docs/zh-TW/` 只是文档。
