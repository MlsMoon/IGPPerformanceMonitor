...
- `src/core/app_config.py` — `%APPDATA%/IGPPerformanceMonitor/config.json` (theme, locale, chart visibility, window/splitter, overlay click-through, monitored processes, timed seconds)
...
- **Clearing a layout is not deleting the widget.** `QLayout.takeAt` only unhooks the item. The widget stays a visible child of the parent until `deleteLater` runs, so a dark `StatsList` sat on top of the light one after Ctrl+D. Hide, `setParent(None)`, then `deleteLater` (`chart_base._clear_layout`). Do not go back to takeAt + deleteLater alone.
...
- **Layout:** `MainWindow._init_ui` only wires `_build_central` / `_build_menu_bar` / `_build_status_bar`; central must be built first because the View menu reads chart state off `MonitorView`. Menu entries go through `_add_action` / `_add_toggle` (toggles are checked **before** `toggled` is connected, so building a menu never fires a slot). Four top-level menus: File · View · Settings · Help. View keeps visibility (Charts submenu, Charts Panel, Show/Hide Overlays). Settings keeps preferences (Dark Mode, Always on Top, Click-through, Language).
- **Menu-bar glyphs** are 16×16 strokes from `theme.text_secondary`, painted by `_GlyphMenuBar` into the item's left padding (`menu_kind` on each `menuAction()`). Fusion draws a menubar item as icon *or* text — never `setIcon` on those actions. Refresh after a theme switch. Do not use `QStyle.standardIcon` (colorful, not flat) and do not add PNG assets for these.
- **Menu text shares one gutter.** View and Settings both mix a submenu with checkable toggles. Qt 5.15.2's stylesheet style indents only checkable rows (QTBUG-90242). `_align_mixed_menu` puts a transparent icon on the plain rows so the labels line up. Call it on both menus. Do not "fix" this with extra `QMenu::item` left padding — that shifts every row and still leaves the delta.
...
- **One overlay per PID** (`MainWindow._overlays`). Cached HWND, ~30fps poll, hide on minimize, `shutdown` on close. Copy through `tr()`. Click-through = `WA_TransparentForMouseEvents` (then the overlay context menu is unreachable — Settings menu is the fallback). F9 sets `set_suppressed`. Bind the PID at start so the overlay can park before the first frame.
